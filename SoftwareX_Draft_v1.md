...
*   **Dynamic Authoring with Generative AI:** Researchers can effortlessly author complex spatial quests by uploading 2D target images and assigning 3D models. To minimize researcher workload and enhance narrative immersion, ARTHunt integrates a multimodal Generative AI (Google Gemini 1.5 Flash) to automatically synthesize poetic, location-based riddles directly from the visual context of the uploaded marker.
*   **Cryptographic Sequential Integrity (Spatial Blockchain):** To ensure the scientific rigor of behavioral data, ARTHunt implements a decentralized, "blockchain-like" sequence validation engine. Each participant is assigned a unique, randomized marker-discovery path upon joining an event. The AR engine enforces this specific sequence in real-time, preventing "herd mentality" bias and ensuring that every participant's spatial navigation data is independent and untainted by group following or sequence-skipping.
