...
--   telemetry(id, ...)             — append-only per-event research events (js/db.js logTelemetry)
...
-- 3. Telemetry Table
-- Append-only: each hunter action (join/scan/wrong_scan/hint/complete) gets
-- its own row instead of being folded into the events.data JSONB blob, so
-- concurrent writes from multiple hunters can't clobber each other.
CREATE TABLE IF NOT EXISTS public.telemetry (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    event_id UUID,
    participant TEXT,
    kind TEXT NOT NULL,        -- join | scan | wrong_scan | hint | complete
    marker INT,
